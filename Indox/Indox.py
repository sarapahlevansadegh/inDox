...
class IndoxRetrievalAugmentation:
    ...
    def create_chunks(self, file_path, content_type=None, unstructured=False, max_chunk_size: Optional[int] = 512):
        """
            ...
            if not unstructured:
                ...
                if do_clustering:
                    all_chunks, input_tokens_all, output_tokens_all = \
                        get_chunks(file_path=file_path,
                    ...
                    all_chunks = get_chunks(file_path=file_path,
                                            embeddings=self.embeddings,
                                            chunk_size=max_chunk_size,
                                            do_clustering=do_clustering,
                                            re_chunk=self.re_chunk)
    ...
    def store_in_vectorstore(self, chunks) -> Any:
        """
        ...
        try:
            if self.db is not None:
                self.db.add_document(docs=chunks, unstructured=self.unstructured)
            return self.db
        except Exception as e:
            print(f"Error while storing in vector store: {e}")
            return None
